  ...
  return (
    <>
      ...
      >
        ...
        <div className="bg-linear-to-r from-green-600 to-green-500 p-5 rounded-t-2xl">
          <div className="flex items-center justify-between">
            ...
            >
              ...
              <div>
                <h1 className="text-slate-900 font-semibold text-lg">Chat with us</h1>
                <p className="text-green-100 text-sm">We typically reply instantly</p>
              </div>
            </div>
            <button 
              onClick={() => setIsOpen(false)}
              className="text-slate-900 hover:bg-white/20 rounded-lg p-1.5 transition-colors"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
        ...
        <div className="p-5 border-t border-gray-100">
          <button
            ...
            className="w-full bg-green-600 hover:bg-green-700 text-slate-900 font-semibold py-3 px-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg hover:shadow-xl transform hover:scale-[1.02]"
          >
      ...
      >
        <div className={`relative transition-all duration-300 ${isHovered ? 'scale-110' : 'scale-100'}`}>
          ...
          <div className="relative h-16 w-16 bg-linear-to-br from-green-500 to-green-600 rounded-full shadow-2xl flex items-center justify-center transform transition-all duration-300 hover:shadow-green-500/50">
             <svg 
              ...
              strokeLinecap="round" 
              strokeLinejoin="round" 
              className={`h-8 w-8 fill-current stroke-none text-slate-900 transition-transform duration-300 ${isOpen ? 'rotate-90 scale-0' : 'rotate-0 scale-100'}`}
            >
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.008-.57-.008-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
            </svg>
            <X className={`h-8 w-8 text-slate-900 absolute transition-transform duration-300 ${isOpen ? 'rotate-0 scale-100' : 'rotate-90 scale-0'}`} />
          </div>
        ...
        {/* Tooltip */}
        <div className={`absolute right-20 bottom-3 bg-gray-900 text-slate-900 px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all duration-300 ${
